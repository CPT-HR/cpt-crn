...
import { getEmployeeFullName, getWorkOrderStatus, canUserEditWorkOrder, transformWorkOrderForPDF } from '@/utils/workOrderHelpers';
...
const WorkOrderView: React.FC = () => {
  ...
  const signatureMetadata = parseSignatureMetadata(workOrder.signature_timestamp, workOrder.signature_coordinates, workOrder.signature_address);
  return <div className="container py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {canEdit && <Button onClick={() => navigate(`/work-orders/${id}/edit`)}>
              <Edit className="h-4 w-4 mr-2" />
              Uredi
            </Button>}
          {workOrder.technician_signature && <Button variant="outline" onClick={handleDownloadPDF}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <span className="font-medium">Datum:</span> {format(new Date(workOrder.date), 'dd.MM.yyyy', {
              locale: hr
            })}
            </div>
            <div>
            ...
            <div>
              <span className="font-medium">Tehničar:</span> {workOrder.employee_profiles ? getEmployeeFullName(workOrder.employee_profiles) : 'N/A'}
            </div>
            {workOrder.arrival_time && <div>
                <span className="font-medium">Vrijeme dolaska:</span> {workOrder.arrival_time}
              </div>}
            {workOrder.completion_time && <div>
        ...
        {workOrder.order_for_customer && <Card>
            <CardHeader>
        ...
        {workOrder.found_condition && <Card>
            <CardHeader>
        ...
        {workOrder.technician_comment && <Card>
            <CardHeader>
        ...
        {workOrder.materials && Array.isArray(workOrder.materials) && workOrder.materials.length > 0 && <Card>
            <CardHeader>
              <CardTitle>Utrošeni materijal</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {workOrder.materials.map((material: any, index: number) => <div key={index} className="flex justify-between">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              {workOrder.technician_signature ? <img src={workOrder.technician_signature} alt="Potpis tehničara" className="max-h-32 border rounded" /> : <div className="text-gray-500">Nema potpisa</div>}
            </div>
            <div>
              <h4 className="font-medium mb-2">Potpis klijenta</h4>
              {workOrder.customer_signature ? <div className="space-y-2">
                  <img src={workOrder.customer_signature} alt="Potpis klijenta" className="max-h-32 border rounded" />
                  {signatureMetadata && <div className="text-[10px] text-gray-500 mt-2 text-center w-full space-y-1">
                      <p className="font-medium text-left">
                        Datum i vrijeme: {signatureMetadata.timestamp}
                      </p>
                      {signatureMetadata.coordinates && <p className="break-all text-left">
                          Koordinate: {signatureMetadata.coordinates.latitude.toFixed(6)}, {signatureMetadata.coordinates.longitude.toFixed(6)}
                        </p>}
                      {signatureMetadata.address && <p className="break-words max-w-full text-left">
                          Lokacija: {signatureMetadata.address}
                        </p>}
                    </div>}
                </div> : <div className="text-gray-500">Nema potpisa</div>}
            </div>
          </CardContent>
        </Card>
      </div>
